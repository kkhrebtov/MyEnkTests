...
namespace TestMailStreamers
    ...
    public static class SqlDataValidator
        ...
        public static void CheckMeetingOutlookResults(String sqlConnectionString, String mailbox, String organizer, List<String> reqAttendees, List<String> optAttendees, String subj, String iCalUid, int duration, List<String> attach, String category)
            {
            String queryString = "SELECT a.name_, a.value_, b.category_ FROM AAL_EVENT_DUMP_EVENTS_DATA as a INNER JOIN AAL_EVENT_DUMP_EVENTS as b ON a.event_id_ = b.event_id_ where b.event_id_ =  (select max(event_id_) from AAL_EVENT_DUMP_EVENTS  where category_ like '%Appointment%')";
            ...
            while (rdr.Read())
                {
                ReadMeetingSingleRow((IDataRecord)rdr, mailbox, organizer, reqAttendees, optAttendees, subj, iCalUid, duration, attach, category);
                }
        ...
        private static void ReadMailSingleRow(IDataRecord record, String mailbox, String fromAddress, List<String> toAddress, String subject, String convId, List<String> attachments, List<String> ccList, List<String> bccList, String category)
            ...
            switch ((String)record[0])
                {
                case "Mailbox":
                    Console.WriteLine("Mailbox actual: " + (String)record[1] + "    Mailbox Expected: "+ fromAddress);
                    if (fromAddress != null) StringAssert.AreEqualIgnoringCase(mailbox, (String)record[1], "Mailbox field verification failed");
                    break;
                case "From":
                    if (toAddress != null) StringAssert.AreEqualIgnoringCase(fromAddress, (String)record[1], "From field verification failed");
                    break;
                case "To":
                    if (toAddress != null) {
